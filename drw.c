...
void
drw_load_fonts(Drw* drw, const char *fonts[], size_t fontcount, const char* preferred_xfontsel) {
    ...
    for (i = 0; i < fontcount; i++) {
        ...
        drw->fonts[0]->set = NULL;
        if (preferred_xfontsel[0] != '\0') {
	if((drw->fonts[0]->set = XCreateFontSet(drw->dpy, preferred_xfontsel, 
           &missing_charset_list_return, &missing_charset_count_return, &def_string_return)) == NULL) 
          { fprintf (stderr, "!!!Unable to create fontset !!!!\n"); } }
}
